...
#else
#include <stdlib.h>             /* for abs() and labs() only */
#include <string.h>
#endif /* __KERNEL__ */
...
long TIC_cntr = TIC_ref, old_TIC_cntr = TIC_ref;
long TIC_sum;
static int nav_count, tic_count = 0, hms_count = 0;
static int astat, mstat, a_missed;
...
gpsisr (void)
{
  ...
  for (ch = 0; ch < N_channels; ch++) {
    ...
    if (test_bit (ch, (void *) &astat)) {
      c->accum.i_dith   = ch_i_track (ch);   /* inphase dither  later code */
  ...
  if (mstat) {
	...
	if ( (ic->state == track) &&
	     ...
	     **/
            meas->doppler_prn =
              ((c->carrier_freq - CARRIER_REF_FREQ) * 31250L / 7)
              & 0xfffffe00L;
	     ...
	     **/
          ...
          meas->doppler_prn |= ic->prn;
          n++;
        }
      }
      measurements.i_TIC_dt = TIC_sum + old_TIC_cntr - TIC_cntr;
      TIC_sum = 0;
    }
    for (ch = 0; ch < N_channels; ch++) {
      struct tracking_channel *c = &chan[ch];
...
ch_pull_in (int ch)
{
  ...
  if ((sign (q_sum) == -1 && c->ms_sign == 0x00000) ||
      (sign (q_sum) == 1 && c->ms_sign == 0xfffff)) {
    if (sign (q_sum) == -sign (c->old_q_sum) 
	&& test_bit (ch, (void *) &a_missed) == 0 
	&& labs (labs (theta) - 25736) < 4096 
	&& labs (labs (c->old_theta) - 25736) < 4096) {
...
ch_track (int ch)
{
  ...
  if (c->ms_count == 19) {
	...
	+ DOP_SIGN * (CARRIER_REF_FREQ - c->carrier_freq) / cc_scale;
      ch_code (ch, c->code_freq);
    }
    c->dfreq1 = c->dfreq;
...
pream (int ch, char bit)
{
  ...
  if (((pream ^ TLM) & 0x3fc00000L) == 0) {     /* preamble pattern found? */
	...
	/* compute the subframe id number */
      ...
      /* synchronize on subframe 1 if TOW matches clock within 300
	 seconds */
        if (sfid_s == 1) {
          TOWs = (HOW & 0x3fffffffL) >> 13;
	  /* ic->sfid = 11; sfid=1 found*/
          d_tow = clock_tow - TOWs * 6 + 5;     /* why the +5? */
          if (labs (d_tow) < 300) {
            c->offset = c->t_count - 59;
        ...
        /* this should improve the re-acquisition time */
          ...
          TOWs = (HOW & 0x3fffffffL) >> 13;
          d_tow = clock_tow - TOWs * 6 + 5;     /* why the +5? */
          /* ic->sfid = 20+sfid_s;  sfid >1 and < 6 found*/
          if (labs (d_tow) < 3) {
